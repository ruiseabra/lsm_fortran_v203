...
C ----------------------------------------------------------------------
C          Version 2.7.1 + 2.0.1 -- OCT 2016 (with snow)
C
...
C ----------------------------------------------------------------------
C NOAH LAND-SURFACE MODEL, UNCOUPLED 1-D COLUMN
C 
C base version: VERSION 2.7.1 July 2004
C
C THE NOAH LSM IS A DESCENDANT OF AN EARLIER GENERATION OF THE OREGON
C STATE UNIVERSITY (OSU) LSM, BUT IT INCLUDES SUBSTANTIAL PHYSICS
C EXTENSIONS AND RECODING ACCOMPLISHED ALONG THE WAY BY NCEP, 
C HL (NWS), AFGWC, AND AFGL/AFPL/AFRL.
C ----------------------------------------------------------------------
C IN 2006 MODIFICATIONS WERE INTRODUCED TO THE MODEL TO SIMULATE
C MUSSEL BEDS, NAMELY BY ADDING NEW VEGETATION AND SOIL TYPES
C 
C modified version: VERSION 1.9.1 November 2006
C David Wethey, Brian Helmuth, Mussel Model 2006
C
C   define BEDDEPTH, CONTACT=contact area, EMISSIVITY in namelist
C   new common blocks  /GEOMETRY/BEDDEPTH,CONTACT
C                      /EMISSIVITY/EMISSIVITY
C                      /TIDE/TIDEFLAG,SST
C ----------------------------------------------------------------------
C LATER, FURTHER MODIFICATIONS WERE INTRODUCED TO MAKE THE CODE 
C FASTER AND SIMPLE
C
C modifications include (but are not limited to):
C     - removal of sea ice flag (now only modeling of land surfaces)
C     - removal of canopy transpiration and dripping
C     - removal of ghost variables (just defined, not used anywhere)
C     - removal of many time variables (the model is run for NRUN
C         timesteps, increasing time by DT steps)
C     - removal of spin-up cycles (model runs only once on each set 
C         of input variables)
C     - removal of all output to files (now output goes to console, and 
C         consists only of temperature for a single layer --> NLAYER)
C     - removal of controlfile and namelist IO (now many variables
C         are hardcoded using R, so that the same lsm layout can be run
C         multiple times without having IO slow down the process)
C     - some variables are now controlled via R code (BODY_DIFUSIVITY, 
C         ANNUAL_BOTTOM_TEMP, ROUGHNESS)
C     - many variables renamed to more intuitive names
C     - vegetation, slope and soil types fixed to match rocky shores,
C         and all associated arrays of values for different types removed
C
C     all together, the new simplified version runs more than 10x faster,
C         which was the main goal for this implementation of the lsm
C 
C     - version 2.7.1 + 3.0.1 is missing all code related to snow, ice
C         and frozen rain (it shouldn't be used when air temperatures 
C         fall below 3-5 C)
C
C modified version: VERSION 2.0.1, "Limpet Model" November 2016
C Rui Seabra, Fernando Lima, António M Santos, David Wethey
C ----------------------------------------------------------------------
C PROGRAM HISTORY LOG
C VERSION 1.0 ------------ MAR 1999
C VERSION 2.7.1 ---------- JUL 2004
C VERSION 2.7.1 + 1.9.1 -- NOV 2006
C Version 2.7.1 + 2.0.1 -- OCT 2016 (with snow)
C Version 2.7.1 + 3.0.1 -- OCT 2016 (without snow)
C ----------------------------------------------------------------------

C VEGETATION TYPE HARDCODED TO THE EQUIVELANT OF 14 IN VERSION 2.7.1 + 1.9.1
C SLOPE      TYPE HARDCODED TO THE EQUIVELANT OF  1 IN VERSION 2.7.1 + 1.9.1
C SOIL       TYPE HARDCODED TO THE EQUIVELANT OF 10 IN VERSION 2.7.1 + 1.9.1
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE PRT_TEMP(TEMPS)
      IMPLICIT NONE
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
CC  PRINT OUTPUT --> TEMPERATURE FOR THE SOIL LAYER NLAYER
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      REAL TEMPS
      REAL TEMPS2
      REAL DIFF
C     Add 30 so that the values are never negative (must subtract later)
      TEMPS2 = TEMPS - 273.15 + 30
...
C     CALCULATE SATURATION MIXING RATIO
      Q2SAT = 0.622 * ESAT / (SFC_PRESS - (1. - 0.622) * ESAT)
...
C---------------------------------------------------------
      ...
      READ (READ_FORCING,*) WIND_SPEED,SFC_TEMP,RH,
     &   SFC_PRESS,SWAVE,LWAVE,RAIN,SST,TIDEFLAG
...
C ----------------------------------------------------------------------
      PARAMETER(CAIR  = 0.7E6)
C     PARAMETER(CAIR  = 1.0E6)
      PARAMETER(CH2O  = 4.184E6)
...
C ----------------------------------------------------------------------
      IF (.NOT. SNOWNG) THEN
        IF (RAIN .GT. 0.0) RR = RR + CPH2O * RAIN / RCH
      ELSE
...
C ----------------------------------------------------------------------
      ...
      SUBROUTINE SFCDIF (ZLM,Z0,T1V,THLM,WIND_SPEED,CZIL,AKMS,AKHS)
      IMPLICIT NONE
C ----------------------------------------------------------------------
...
C ----------------------------------------------------------------------
      ...
      REAL RIC, RRIC, FHNEU, RFC, RFAC, ZZ
      REAL XX, PSPMU, YY, PSPMS, PSPHU, PSPHS, ZLM, Z0, T1V, THLM
      REAL WIND_SPEED, CZIL, AKMS, AKHS, ZILFC, ZU, ZT, RDZ, CXCH
...
C ----------------------------------------------------------------------
      ...
      ELSE
        IF (RAIN .GT. 0.0) FLX1 = CPH2O * RAIN * (TSKIN - SFC_TEMP)
      ENDIF
C ----------------------------------------------------------------------
